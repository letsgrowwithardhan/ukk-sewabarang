...
$hasil=mysqli_query($konek,"SELECT tbpeminjaman.no_transaksi,tbuser.id_user,tbuser.nama,
    tbbarang.id_barang,tbbarang.nama_barang,tbpeminjaman.jml_pinjam,tbpeminjaman.tgl_pinjam,tbpeminjaman.tgl_batas_kembali, tbpeminjaman.status  
    from tbpeminjaman,tbbarang,tbuser where tbpeminjaman.id_user=tbuser.id_user 
    AND tbpeminjaman.id_barang=tbbarang.id_barang AND tbpeminjaman.status='pending'");
...
<?php
    echo "
    <td align=center><button><a href=konfirmasi_peminjaman.php?no_transaksi=$baris[no_transaksi] onclick='return confirm(\"Anda yakin ubah status data pinjam $baris[no_transaksi]?\")'>Konfirmasi</a></button></td>
    <td align=center><button><a href=tolak_peminjaman.php?no_transaksi=$baris[0] onclick='return confirm(\"Anda yakin data barang $baris[no_transaksi] akan ditolak?\")'>Tolak</a></button></td></tr></tbody>";
    $no++;
    }
    echo "</table>";
    ?>
    <form action=cetakpinjam.php method=post class="print">
